...
class CurlPrintessHttpAdapter implements PrintessHttpAdapterInterface
{
    ...
    public function send($httpMethod, $url, $headers, $httpBody): ?ResponseInterface
    {
        ...
        switch ($httpMethod) {
            case PrintessApiClient::HTTP_POST:
                curl_setopt($curl, CURLOPT_POST, true);
                curl_setopt($curl, CURLOPT_POSTFIELDS,  $httpBody);

                break;
            case PrintessApiClient::HTTP_GET:
                break;
            case PrintessApiClient::HTTP_PATCH:
                curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'PATCH');
                curl_setopt($curl, CURLOPT_POSTFIELDS, $httpBody);

                break;
            case PrintessApiClient::HTTP_DELETE:
    ...
    protected function parseResponseBody(string $response, int $statusCode, string $httpBody): ?stdClass
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new ApiException("Unable to decode Printess response: '$response'.");
        }
